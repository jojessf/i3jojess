...
chomp($hostname);
$opt->{r} //= "1920x1080";
$opt->{d} //= "HDMI-1-0" if $hostname =~ /myou/;
$opt->{d} //= "HDMI-1" if $hostname =~ /yori/;
$opt->{d} //= "HDMI-0" if $hostname =~ /senzai/;
$opt->{do} //= "DP-1-1" if $hostname =~ /myou/;
$opt->{do} //= "eDP-1" if $hostname =~ /yori/i;
$opt->{do} //= "HDMI-1" if $hostname =~ /senzai/;
$opt->{c} //= "xrandr --output ".$opt->{d} . " ";
$opt->{pos} //= 2 if $hostname =~ /myou/;
$opt->{pos} //= 1 if $hostname =~ /senzai/;
...
$opt->{v} //= $ENV{DEBUG};
$opt->{v} //= 0;
$opt->{postcom} //= "xrandr --output " . $opt->{d} ." --primary";
